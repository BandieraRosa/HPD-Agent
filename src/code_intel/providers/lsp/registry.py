...
class LanguageServerSpec(BaseModel):
    ...
    language: str = Field(
        description="Code-intel language identifier handled by this server."
    )
    name: str = Field(description="Human-readable language server name.")
    detect_command: list[str] = Field(
        description="Command shape used to detect whether the server executable exists."
    )
    launch_command: list[str] = Field(
        description="Command used to launch the stdio language server."
    )
    install_hint: str = Field(
        description="Exact install command shown when the executable is missing."
    )
    root_markers: list[str] = Field(
        description="Workspace-root marker filenames for this server."
    )
...
def default_language_server_specs() -> dict[str, LanguageServerSpec]:
    ...
    return {
        spec.language: spec.model_copy(deep=True)
        for spec in DEFAULT_LANGUAGE_SERVER_SPECS
    }


def language_server_specs(
    specs: Iterable[LanguageServerSpec] | None = None,
) -> dict[str, LanguageServerSpec]:
