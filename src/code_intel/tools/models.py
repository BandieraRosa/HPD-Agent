...
class SearchMatch(BaseModel):
    ...
    symbol_id: str | None = Field(
        default=None, description="Stable symbol ID when the match is a symbol."
    )
    name: str = Field(
        description="Matched symbol name, or the query label for text matches."
    )
    qualified_name: str | None = Field(
        default=None, description="Qualified symbol name when available."
    )
    kind: str = Field(
        description="English match kind, such as function, class, method, or text."
    )
    ...
    confidence: float = Field(
        ge=0.0, le=1.0, description="Provider confidence from 0.0 to 1.0."
    )
...
class CodeSearchData(BaseModel):
    ...
    matches: list[SearchMatch] = Field(
        default_factory=list, description="Normalized search matches."
    )
...
class CodeOutlineData(BaseModel):
    ...
    symbols: list[Symbol] = Field(
        default_factory=list, description="File symbols, nested by parent_id."
    )
    line_count: int = Field(
        ge=0, description="Stable estimated line count derived from symbol ranges."
    )
...
class CodeContextData(BaseModel):
    ...
    signature: str | None = Field(
        default=None, description="Target signature when requested."
    )
    body: str | None = Field(default=None, description="Target body when requested.")
    parents: list[Symbol] = Field(
        default_factory=list, description="Outer symbols such as classes or modules."
    )
    imports: list[str] = Field(
        default_factory=list, description="Relevant import lines."
    )
    nearby_symbols: list[Symbol] = Field(
        default_factory=list, description="Nearby same-file symbols."
    )
    truncated: bool = Field(
        default=False, description="Whether token budgeting truncated the context."
    )
...
class CodeSemanticData(BaseModel):
    ...
    locations: list[Location] = Field(
        default_factory=list, description="Definition or reference locations."
    )
    hover: HoverInfo | None = Field(
        default=None, description="Hover result when requested."
    )
    document_symbols: list[Symbol] = Field(
        default_factory=list, description="Document symbols when requested."
    )
    ...
    more_available: bool = Field(
        default=False, description="Whether result limits hid additional data."
    )
...
class CodeVerifyData(BaseModel):
    ...
    new_diagnostics: list[Diagnostic] = Field(
        default_factory=list, description="Diagnostics considered new."
    )
    resolved_diagnostics: list[Diagnostic] = Field(
        default_factory=list, description="Diagnostics considered resolved."
    )
    unchanged_diagnostics: list[Diagnostic] = Field(
        default_factory=list, description="Diagnostics unchanged from baseline."
    )
    severity_delta: dict[str, int] = Field(
        default_factory=dict, description="Severity deltas keyed by English severity."
    )
    checks_run: list[str] = Field(
        default_factory=list, description="Checks that were actually executed."
    )
    checks_skipped: list[ChecksSkipped] = Field(
        default_factory=list, description="Checks intentionally skipped."
    )
    recommended_next_action: Literal["proceed", "repair", "abort"] = Field(
        description="Recommended next step for the agent.",
    )
    call_source: Literal["agent", "workflow"] = Field(
        default="agent", description="Verification call-source bucket."
    )
    ...
    baseline_key: str | None = Field(
        default=None, description="Verifier baseline cache key used for this call."
    )
    baseline_refreshed: bool = Field(
        default=False, description="Whether this call captured a new agent baseline."
    )
    provider_error: ToolError | None = Field(
        default=None, description="Safe provider error when verification is partial."
    )
