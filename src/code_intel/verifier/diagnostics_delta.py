...
def _jsonable(value: object) -> object:
    ...
    if isinstance(value, set):
        ...
        return sorted((_jsonable(item) for item in st), key=lambda item: json.dumps(item, sort_keys=True))
...
def content_hash_for_paths(workspace_root: str | Path, relevant_paths: Iterable[str]) -> str:
...
class PatchLineEdit(BaseModel):
    ...
    start_line: int = Field(ge=0, description="0-based original start line for the edit.")
    old_line_count: int = Field(ge=0, description="Number of original lines replaced or deleted.")
    new_line_count: int = Field(ge=0, description="Number of replacement or inserted lines.")
...
class PatchSummary(BaseModel):
    ...
    edits: list[PatchLineEdit] = Field(default_factory=list, description="Patch line edits in original file coordinates.")
...
class BaselineKey(BaseModel):
    ...
    relevant_paths: tuple[str, ...] = Field(description="Sorted workspace-relative paths in this baseline scope.")
    content_hash: str = Field(description="Hash of the relevant path contents at baseline capture time.")
    provider_id: str = Field(description="Stable provider identity label used for diagnostics.")
    provider_config_hash: str = Field(description="Hash of provider configuration that can affect diagnostics.")
    call_source: CallSource = Field(description="Caller bucket; workflow and agent baselines are isolated.")
...
class BaselineSnapshot(BaseModel):
    ...
    diagnostics: list[Diagnostic] = Field(default_factory=list, description="Diagnostics captured for this baseline.")
...
def create_baseline_snapshot(
    *,
    ...
    return BaselineSnapshot(
        ...
        symbols_by_path={path: list(symbols) for path, symbols in (symbols_by_path or {}).items()},
    )
...
def refresh_workflow_baseline(
    *,
    ...
    snapshot = create_baseline_snapshot(key=workflow_key, diagnostics=diagnostics, symbols_by_path=symbols_by_path)
...
def refresh_agent_baseline(
    *,
    ...
    snapshot = create_baseline_snapshot(key=agent_key, diagnostics=diagnostics, symbols_by_path=symbols_by_path)
    return (cache or _BASELINE_CACHE).refresh(snapshot)


def get_cached_baseline(key: BaselineKey, *, exact: bool = False, cache: BaselineCache | None = None) -> BaselineSnapshot | None:
...
class DeltaDiagnostic(BaseModel):
    ...
    normalized_message: str = Field(description="Diagnostic message after fixed-rule normalization.")
    semantic_anchor: str = Field(description="Enclosing symbol anchor, shifted-line fallback, or <deleted>.")
    drifted_start_line: int = Field(ge=0, description="Baseline line after applying PatchSummary drift.")
    fingerprint: str = Field(description="Stable fingerprint over path/code/normalized message/semantic anchor.")


class DiagnosticsDelta(BaseModel):
    """Three-way diagnostics delta between a baseline and post-patch result."""

    new: list[DeltaDiagnostic] = Field(default_factory=list, description="Diagnostics present only after the patch.")
    resolved: list[DeltaDiagnostic] = Field(default_factory=list, description="Baseline diagnostics absent after the patch.")
    unchanged: list[DeltaDiagnostic] = Field(default_factory=list, description="Diagnostics present before and after.")
    severity_delta: dict[str, int] = Field(default_factory=lambda: {severity: 0 for severity in _SEVERITIES})
    partial: bool = Field(default=False, description="True when verification is usable but incomplete/non-blocking.")
    provider_error: ToolError | None = Field(default=None, description="Safe provider error when diagnostics are unavailable.")
    ...
    def has_new_errors(self) -> bool:
        return any(item.diagnostic.severity == DiagnosticSeverity.ERROR for item in self.new)

    @property
    def has_new_warnings(self) -> bool:
        return any(item.diagnostic.severity == DiagnosticSeverity.WARNING for item in self.new)
...
def compute_delta(
    ...
    *,
    ...
    with trace_span("code_intel.verifier.compute_delta", _diagnostic_trace_input_metadata(baseline, after)) as span:
...
def _compute_delta_untraced(
    ...
    *,
    ...
    if isinstance(baseline, BaselineSnapshot):
        ...
        baseline_symbols = _symbol_map(baseline_symbols_by_path or baseline.symbols_by_path)
    else:
    ...
    after_symbol_names = _symbol_names_by_path(after_symbols) if after_symbols_by_path is not None else None
    ...
    return DiagnosticsDelta(new=new, resolved=resolved, unchanged=unchanged, severity_delta=severity_delta, partial=partial)
...
def _diagnostic_trace_input_metadata(
    ...
    diagnostics = list(baseline.diagnostics if isinstance(baseline, BaselineSnapshot) else baseline) + list(after)
    return {
...
def _delta_trace_metadata(delta: DiagnosticsDelta) -> dict[str, object]:
    diagnostics = delta.new_diagnostics + delta.resolved_diagnostics + delta.unchanged_diagnostics
    metadata: dict[str, object] = {
        "result_count": len(diagnostics),
        "truncated": delta.partial,
        "paths": [diagnostic.path for diagnostic in diagnostics],
        "diagnostic_templates": [item.normalized_message for item in delta.new + delta.resolved + delta.unchanged],
    }
...
def _symbol_map(symbols_by_path: Mapping[str, Iterable[Symbol]]) -> dict[str, list[Symbol]]:
    return {validate_workspace_relative_path(path): list(symbols) for path, symbols in symbols_by_path.items()}


def _symbol_names_by_path(symbols_by_path: Mapping[str, list[Symbol]]) -> dict[str, set[str]]:
...
def _anchor_diagnostic(
    ...
    *,
    ...
    drifted_line = patch_summary.shifted_line(diagnostic.path, diagnostic.range.start_line)
    anchor, symbol_name = _semantic_anchor(symbols_by_path.get(diagnostic.path, []), diagnostic.path, diagnostic.range)
    ...
    fingerprint = _diagnostic_fingerprint(diagnostic, normalized_message, semantic_anchor)
...
def _semantic_anchor(symbols: list[Symbol], path: str, source_range: Range) -> tuple[str | None, str | None]:
    candidates = [symbol for symbol in symbols if symbol.path == path and _range_contains(symbol.range, source_range)]
    if not candidates:
        return None, None
...
def _diagnostic_fingerprint(diagnostic: Diagnostic, normalized_message: str, semantic_anchor: str) -> str:
    return stable_json_hash(
        {
...
def _range_size(source_range: Range) -> tuple[int, int]:
    return (source_range.end_line - source_range.start_line, source_range.end_col - source_range.start_col)


def _severity_delta(new: list[DeltaDiagnostic], resolved: list[DeltaDiagnostic]) -> dict[str, int]:
