...
from src.code_intel.core import Capability, CodeTarget, Location, ProviderStatus, ProviderUnavailable, Range
...
class _FakeSemanticClient:
    def __init__(
        ...
        *,
        ...
        self.definition_result: list[Location] = definition_result or [_definition_location()]
    ...
    async def initialize(
        ...
        *,
        ...
        return lsp_types.InitializeResult(capabilities=lsp_types.ServerCapabilities(definition_provider=True))
...
def test_crash_marks_unhealthy_kernel_skips_and_restart_success_restores_health(tmp_path: Path) -> None:
    async def scenario() -> None:
        crashing = _FakeSemanticClient(definition_error=ProviderUnavailable("language server crashed"))
        recovered = _FakeSemanticClient(definition_result=[_definition_location()])
        factory = _SequenceFactory([crashing, recovered])
        provider = LSPProvider(tmp_path, manager=LSPManager(tmp_path, specs=[_spec()], client_factory=factory))
...
def test_silent_stop_is_detected_by_health_before_kernel_invokes_semantics(tmp_path: Path) -> None:
    async def scenario() -> None:
        silently_stopped = _FakeSemanticClient(definition_result=[_definition_location()])
...
def test_three_restart_failures_become_permanent_and_explicit_success_clears_it(tmp_path: Path) -> None:
    async def scenario() -> None:
        crashing = _FakeSemanticClient(definition_error=ProviderUnavailable("language server crashed"))
        failures = [
            _FakeSemanticClient(initialize_error=ProviderUnavailable("restart failed one")),
            _FakeSemanticClient(initialize_error=ProviderUnavailable("restart failed two")),
            _FakeSemanticClient(initialize_error=ProviderUnavailable("restart failed three")),
        ]
        recovered = _FakeSemanticClient(definition_result=[_definition_location()])
        ...
        assert permanent.message is not None and "permanently_unhealthy" in permanent.message
